...
export default function App() {
  ...
  const [storyCustomStyle, setStoryCustomStyle] = useState('');
      ...
      <div className="w-96 flex flex-col gap-4 overflow-hidden">
        <div className="flex-1 glass-panel flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6">
            ...
            {activeTab === 'story' && (
              <div className="space-y-6">
                <div className="p-8 border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center text-center gap-4 hover:bg-white/5 transition-all cursor-pointer group"
                ...
                >
                  ...
                  <div>
                    <p className="text-xs font-bold uppercase tracking-widest mb-1">Dosya Yükle</p>
                    <p className="text-[10px] text-zinc-500">PDF, DOCX veya TXT sürükleyin</p>
                  </div>
                  <input type="file" accept=".pdf,.docx,.txt" className="hidden" id="story-upload" onChange={(e) => { const file = e.target.files?.[0]; if (file) handleTextUpload(file); }} />
                  <label htmlFor="story-upload" className="px-6 py-2 bg-white/5 hover:bg-white/10 rounded-lg text-[10px] font-bold uppercase tracking-widest cursor-pointer transition-all">Dosya Seç</label>
                </div>
              </div>
            )}
            ...
            {activeTab === 'audio' && (
              <div className="space-y-6">
                <div className="p-6 border border-white/5 rounded-2xl bg-black/20 text-center space-y-4">
                  <div className="w-16 h-16 bg-purple-500/10 rounded-full flex items-center justify-center mx-auto">
                    <Music className="w-8 h-8 text-purple-400" />
                  </div>
                  <div>
                    <h3 className="text-sm font-bold text-white mb-2">Ses Dosyasından Görsele</h3>
                    <p className="text-[10px] text-zinc-400 leading-relaxed">
                      Yüklediğiniz ses dosyasını analiz ederek otomatik olarak sahneler çıkarır ve her sahne için görsel üretir.
                    </p>
                  </div>
                  <button 
                    onClick={() => setIsAudioPipelineOpen(true)}
                    className="w-full py-4 bg-gradient-to-r from-purple-600 to-blue-600 rounded-xl flex items-center justify-center gap-3 group overflow-hidden relative mt-4"
                  >
                    <div className="absolute inset-0 bg-white/20 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700" />
                    <Music className="w-5 h-5" />
                    <span className="text-xs font-bold uppercase tracking-widest">Audio Pipeline'ı Aç</span>
                  </button>
                </div>
              </div>
            )}
            ...
            {activeTab === 'cinema' && (
              <div className="space-y-6">
                  ...
                  <div className="aspect-video rounded-xl border border-white/10 bg-black/50 overflow-hidden relative group">
                    {cinemaSceneRef ? (
                      <>
                        ...
                        <button onClick={() => setCinemaSceneRef(null)} className="absolute top-2 right-2 p-1.5 bg-black/60 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity"><X className="w-4 h-4" /></button>
                      </>
                    ) : (
                      <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer hover:bg-white/5 transition-all">
                        ...
                        <input type="file" accept="image/*" className="hidden" onChange={(e) => {
                          ...
                          if (file) {
                            const reader = new FileReader();
                            reader.onload = (ev) => setCinemaSceneRef(ev.target?.result as string);
                            reader.readAsDataURL(file);
                          }
                        }} />
                      </label>
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Karakter Referansı</label>
                  <div className="aspect-square w-24 rounded-xl border border-white/10 bg-black/50 overflow-hidden relative group">
                    {cinemaCharRef ? (
                      <>
                        <img src={cinemaCharRef} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                        <button onClick={() => setCinemaCharRef(null)} className="absolute top-2 right-2 p-1.5 bg-black/60 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity"><X className="w-4 h-4" /></button>
                      </>
                    ) : (
                      <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer hover:bg-white/5 transition-all">
                        <Plus className="w-6 h-6 text-zinc-700" />
                        <input type="file" accept="image/*" className="hidden" onChange={(e) => {
                          ...
                          if (file) {
                            const reader = new FileReader();
                            reader.onload = (ev) => setCinemaCharRef(ev.target?.result as string);
                            reader.readAsDataURL(file);
                          }
                        }} />
                      </label>
                ...
                <button 
                  ...
                  className="w-full py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all disabled:opacity-30"
                >
                  {isCinemaAnalyzing ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : 'Sahneyi Analiz Et'}
                </button>
                      ...
                      {cinemaAnalysis.map((shot, i) => (
                        <div key={i} className="flex items-center gap-3 p-3 bg-white/5 border border-white/5 rounded-xl">
                          ...
                          <div className="flex-1">
                            <p className="text-[10px] font-bold">{shot.shot_name_tr}</p>
                            <p className="text-[8px] text-zinc-500 uppercase">{shot.shot_type}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                    <button onClick={handleCinemaStart} className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-black font-bold rounded-xl uppercase text-xs tracking-widest shadow-lg shadow-emerald-500/20 transition-all">Çekimleri Başlat</button>
                  </div>
                )}
              </div>
            )}
            ...
            >
              <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/5">
                <h2 className="text-lg font-bold flex items-center gap-2">
                  <FileText className="w-5 h-5 text-purple-400" /> Senaryo Üretim Ayarları
                </h2>
                <button onClick={() => setIsStoryModalOpen(false)}><X className="w-6 h-6 text-zinc-500" /></button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8">
                <div className="space-y-3">
                  ...
                  <textarea 
                    ...
                    placeholder="Senaryoya eklemek istediğiniz ekstra detaylar veya hayaliniz..."
                    className="w-full h-20 bg-black/50 border border-white/10 rounded-xl p-4 text-xs focus:outline-none focus:border-blue-500 transition-all font-mono leading-relaxed resize-none"
                  />
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <textarea 
                    ...
                    className="w-full h-48 bg-black/50 border border-white/10 rounded-xl p-4 text-xs focus:outline-none focus:border-purple-500 transition-all font-mono leading-relaxed"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-3">
                    ...
                    {storyAr === 'manual' && (
                      <div className="flex items-center gap-2 mt-2">
                        <input 
                          type="number" 
                          value={storyManualAr.width}
                          onChange={(e) => setStoryManualAr({ ...storyManualAr, width: Number(e.target.value) })}
                          className="w-full bg-black/50 border border-white/10 rounded-lg px-3 py-1.5 text-xs text-center focus:outline-none focus:border-purple-500"
                          placeholder="Genişlik"
                        />
                        <span className="text-zinc-500 text-xs">x</span>
                        <input 
                          type="number" 
                          value={storyManualAr.height}
                          onChange={(e) => setStoryManualAr({ ...storyManualAr, height: Number(e.target.value) })}
                          className="w-full bg-black/50 border border-white/10 rounded-lg px-3 py-1.5 text-xs text-center focus:outline-none focus:border-purple-500"
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Üretim Adedi</label>
                    <input 
                      type="number" 
                      value={storyImgCount}
                      onChange={(e) => setStoryImgCount(Number(e.target.value))}
                      min={1} max={10}
                      className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-purple-500"
                    />
                  </div>
                </div>
                    ...
                    <div className="aspect-square rounded-xl border border-white/10 bg-black/50 overflow-hidden relative group">
                      {storySceneRef ? (
                        ...
                        <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer hover:bg-white/5 transition-all">
                          ...
                          <span className="text-[10px] text-zinc-500 uppercase font-bold">Yükle</span>
                          <input type="file" accept="image/*" className="hidden" onChange={(e) => {
                            const file = e.target.files?.[0];
                            if (file) {
                              const reader = new FileReader();
                              reader.onload = (ev) => setStorySceneRef(ev.target?.result as string);
                              reader.readAsDataURL(file);
                            }
                          }} />
                        </label>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="aspect-square rounded-xl border border-white/10 bg-black/50 overflow-hidden relative group">
                      {storyCharRef ? (
                        ...
                        <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer hover:bg-white/5 transition-all">
                          <Camera className="w-6 h-6 text-zinc-600 mb-2" />
                          <span className="text-[10px] text-zinc-500 uppercase font-bold">Yükle</span>
                          <input type="file" accept="image/*" className="hidden" onChange={(e) => {
                            const file = e.target.files?.[0];
                            if (file) {
                              const reader = new FileReader();
                              reader.onload = (ev) => setStoryCharRef(ev.target?.result as string);
                              reader.readAsDataURL(file);
                            }
                          }} />
                        </label>
              ...
              <div className="p-6 border-t border-white/5 bg-black/20 flex gap-4">
                <button onClick={() => setIsStoryModalOpen(false)} className="flex-1 py-3 bg-white/5 hover:bg-white/10 rounded-xl font-bold uppercase text-xs tracking-widest transition-all">İptal</button>
                <button onClick={handleStoryStart} className="flex-[2] py-3 bg-purple-600 hover:bg-purple-500 rounded-xl font-bold uppercase text-xs tracking-widest transition-all shadow-lg shadow-purple-500/20">Analiz Et ve Başlat</button>
              </div>
            </motion.div>
          </motion.div>
        )}
          ...
          >
            <div className="p-6 border-b border-white/5 flex items-center justify-between bg-black/20">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-bold">Audio Pipeline</h2>
                  <p className="text-[10px] text-zinc-500 uppercase tracking-widest">Ses Dosyasından Tam Otomatik Görsel Hikaye</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <button 
                  ...
                  className="px-8 py-3 bg-emerald-500 hover:bg-emerald-400 text-black font-bold rounded-xl uppercase text-xs tracking-widest transition-all disabled:opacity-30"
                >
                  ▶ Tümünü Başlat
                </button>
                <button onClick={() => setIsAudioPipelineOpen(false)} className="p-3 hover:bg-white/10 rounded-full transition-colors"><X className="w-6 h-6" /></button>
              </div>
            </div>

            <div className="flex-1 grid grid-cols-5 gap-4 p-6 overflow-hidden">
              {[
